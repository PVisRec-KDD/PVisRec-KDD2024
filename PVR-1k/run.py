...
def offlineEvaluate(mab, user, n_arms, rewards, rewards_ATTR,rewards_CFG, contexts, nrounds, noise=0.05):
	...
	for i in range(nrounds):
		...
		regret+=1-vrew
		cum_regret[i]=(regret)
		action_list.append((user,i,a1,a2,c))
		# print('loop over')
	...
	for user,visList in tqdm(user2Vis.items()):
		if user in user2AttrAll.keys():
			...
			if len(attrList)<20:
				...
				note_reward_ConUCB, cum_regret_ConUCB, act_ConUCB = offlineEvaluate(mab1, user, arms, rewards, ATTR_reward, CFG_reward, contexts, n_rounds,noise=0.05)
				# print('test')
				note_reward_LinUCB, cum_regret_LinUCB, act_LinUCB = offlineEvaluate(mab2, user, arms, rewards, ATTR_reward, CFG_reward, contexts, n_rounds,noise=0.05)
				note_reward_SemiUCB, cum_regret_SemiUCB, act_SemiUCB = offlineEvaluate(mab3, user, arms, rewards, ATTR_reward, CFG_reward, contexts, n_rounds,noise=0.05)	
				# results_SemiNoBias, note_reward_SemiNoBias, cum_regret_SemiNoBias, _ = offlineEvaluate(mab4, user, arms, rewards, ATTR_reward, CFG_reward, contexts, n_rounds,noise=0.05)	
				# results_SemiNoHier, note_reward_SemiNoHier, cum_regret_SemiNoHier, _ = offlineEvaluate(mab5, user, arms, rewards, ATTR_reward, CFG_reward, contexts, n_rounds,noise=0.05)	
